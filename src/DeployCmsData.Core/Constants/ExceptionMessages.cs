namespace DeployCmsData.Core.Constants
{
    public static class ExceptionMessages
    {
        public const string ParentAliasNotFound = "Parent alias not found";
        public const string DocumentTypeNotFound = "Cannot find document type by alias";
        public const string CannotCreateDocumentType = "Cannot create Document Type";
        public const string ParentFolderNotFound = "Parent folder not found";
        public const string PropertyAliasAlreadyExists = "Property alias already exists";
        public const string AliasNotDefined = "Alias cannot be empty";
        public const string ParentFolderNameNotDefined = "Parent folder name cannot be empty";
        public const string UpgradeScriptIsNull = "Upgrade script is null";
        public const string CannotFindDataType = "Cannot find the data type: ";
    }
}